    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            using (var client = new HttpClient())
            {
                ...
                var slackUrl = "https://hooks.slack.com/services/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";
                foreach (var eventObj in jsonDes.events)
                {
                    var contentJson = $"{{\"text\":\"{eventObj.title}\n{eventObj.event_url}\"}}";
                    var content = new StringContent(contentJson, Encoding.UTF8, @"application/json");
                    var result1 = await client.PostAsync(slackUrl, content);
                }
